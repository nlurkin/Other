...
\begin{enumerate}[label=\textbf{CMD.\arabic*}]
	...
	\item \label{cmd:endTranfer} dimServerName/EndTranfer (Format: ``I''): For signaling that the
	last configuration file has been tranfered.
\end{enumerate}
...
The client part can connect to a server's command port to send commands to the server or to a
server's service port to receive data from the server. The RunControl is acting as a client
connected to the device server whose minimum interface is described above, but is also acting as a
server, providing different informations:
\begin{itemize}
	...
	(0x18) defined in \cite{biblio:TDAQNote} and NSource is the number of data source for this detector. This
...
runs, parameters that rarely change. The following files would contain paramParler du fait qu'ils peuvent prendre des trucs chez moi aussieters that have a
...
\begin{enumerate}
	...
	\ref{cmd:endTranfer} to warn the device that there are no more configuration files.
	\item The device is now supposed to report back the current configuration. It will generate a file
	containing all the current parameters values, possibly in the same format as the configuration
...
service port \ref{svc:logging}. This service provides the source id, a severity code and finally
...
\begin{enumerate}[label=\textbf{ASVC.\arabic*}]
	\item \label{svc:stateParams} dimServerName/paramName: An alternative to the single \ref{svc:state}
	service reporting the FSM state that is internally maintained and has to comply with the
	convention outline in Table \ref{table:FSMStates} is to provide a serie of parameters value via as
	much service ports. The RunControl can assemble these values through logical expression to
	determine one of the state depicted in figure \ref{fig:FSM_Device}. For the rest, these ports have
